...
4. Failed opens are cleaned up, not cached, so a later dispatch can retry.
5. Writes to the same connector are serialized; independent connector IDs may progress concurrently.
