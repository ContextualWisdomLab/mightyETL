...
- Production container builds now use digest-pinned Docker base images while retaining explicit Maven/Temurin tags, so reviewed build inputs cannot silently drift when upstream tags move; image refresh remains an explicit reviewed update rather than an automatic security claim.
